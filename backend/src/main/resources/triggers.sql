...
-- Create a function to update AlarmSystem reliability when a vehicle is removed from wanted list
CREATE OR REPLACE FUNCTION update_alarm_system_reliability_on_found()
RETURNS TRIGGER AS $$
DECLARE
    alarm_system_id BIGINT;
    theft_count BIGINT;
    ...
    theft_percentage FLOAT;
BEGIN
    -- Get the alarm_system_id of the vehicle being removed from wanted list
    SELECT v.alarm_system_id INTO alarm_system_id
    FROM vehicle v
    WHERE v.id = NEW.vehicle_id;
    
    -- Only proceed if we have a valid alarm_system_id and the status changed to FOUND
    IF alarm_system_id IS NOT NULL AND NEW.status = 'FOUND' AND OLD.status = 'WANTED' THEN
        -- Count the number of thefts for vehicles with this alarm system
        SELECT COUNT(*) INTO theft_count
        FROM wanted_vehicles wv
        JOIN vehicle v ON wv.vehicle_id = v.id
